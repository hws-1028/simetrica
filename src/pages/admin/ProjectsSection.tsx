import { useState, useEffect } from 'react';
import Table from '../../components/admin/Table';
import Modal from '../../components/admin/Modal';
import type { TableColumn } from '../../components/admin/Table';
import type { Project, ProjectFormData } from '../../types/project.types';
import adminProjectService from '../../services/adminProjectService';
import './ProjectsSection.css';

export default function ProjectsSection() {
  const [projects, setProjects] = useState<Project[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedProject, setSelectedProject] = useState<Project | null>(null);
  const [showFormModal, setShowFormModal] = useState(false);
  const [editingProject, setEditingProject] = useState<Project | null>(null);
  
  // Form state
  const [formData, setFormData] = useState<ProjectFormData>({
    nombre: '',
    cliente: '',
    descripcion: '',
    ubicacion: '',
    duracion: '',
    personasInvolucradas: 1,
    imagenes: [],
  });
  const [formErrors, setFormErrors] = useState<Record<string, string>>({});
  const [submitting, setSubmitting] = useState(false);

  useEffect(() => {
    loadProjects();
  }, []);

  const loadProjects = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await adminProjectService.getAll(1, 50);
      setProjects(response.projects);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Error al cargar proyectos');
    } finally {
      setLoading(false);
    }
  };

  const handleDelete = async (id: string) => {
    if (!confirm('¿Estás seguro de eliminar este proyecto?')) return;

    try {
      await adminProjectService.delete(id);
      setProjects((prev) => prev.filter((p) => p._id !== id));
      if (selectedProject?._id === id) {
        setSelectedProject(null);
      }
    } catch {
      alert('Error al eliminar el proyecto');
    }
  };

  const openCreateModal = () => {
    setEditingProject(null);
    setFormData({
      nombre: '',
      cliente: '',
      descripcion: '',
      ubicacion: '',
      duracion: '',
      personasInvolucradas: 1,
      imagenes: [],
    });
    setFormErrors({});
    setShowFormModal(true);
  };

  const openEditModal = (project: Project) => {
    setEditingProject(project);
    setFormData({
      nombre: project.nombre,
      cliente: project.cliente,
      descripcion: project.descripcion,
      ubicacion: project.ubicacion,
      duracion: project.duracion,
      personasInvolucradas: project.personasInvolucradas,
      imagenes: project.imagenes.map(img => img._id),
    });
    setFormErrors({});
    setShowFormModal(true);
  };

  const validateForm = (): boolean => {
    const errors: Record<string, string> = {};

    if (!formData.nombre.trim()) {
      errors.nombre = 'El nombre es requerido';
    } else if (formData.nombre.length > 200) {
      errors.nombre = 'El nombre no puede exceder 200 caracteres';
    }

    if (!formData.cliente.trim()) {
      errors.cliente = 'El cliente es requerido';
    } else if (formData.cliente.length > 200) {
      errors.cliente = 'El cliente no puede exceder 200 caracteres';
    }

    if (!formData.descripcion.trim()) {
      errors.descripcion = 'La descripción es requerida';
    } else if (formData.descripcion.length > 2000) {
      errors.descripcion = 'La descripción no puede exceder 2000 caracteres';
    }

    if (!formData.ubicacion.trim()) {
      errors.ubicacion = 'La ubicación es requerida';
    } else if (formData.ubicacion.length > 200) {
      errors.ubicacion = 'La ubicación no puede exceder 200 caracteres';
    }

    if (!formData.duracion.trim()) {
      errors.duracion = 'La duración es requerida';
    } else if (formData.duracion.length > 100) {
      errors.duracion = 'La duración no puede exceder 100 caracteres';
    }

    if (formData.personasInvolucradas < 1) {
      errors.personasInvolucradas = 'Debe haber al menos 1 persona involucrada';
    }

    setFormErrors(errors);
    return Object.keys(errors).length === 0;
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();

    if (!validateForm()) {
      return;
    }

    try {
      setSubmitting(true);
      
      if (editingProject) {
        const response = await adminProjectService.update(editingProject._id, formData);
        setProjects((prev) =>
          prev.map((p) => (p._id === response.project._id ? response.project : p))
        );
      } else {
        const response = await adminProjectService.create(formData);
        setProjects((prev) => [response.project, ...prev]);
      }

      setShowFormModal(false);
      setFormData({
        nombre: '',
        cliente: '',
        descripcion: '',
        ubicacion: '',
        duracion: '',
        personasInvolucradas: 1,
        imagenes: [],
      });
    } catch (err) {
      alert(err instanceof Error ? err.message : 'Error al guardar el proyecto');
    } finally {
      setSubmitting(false);
    }
  };

  const columns: TableColumn<Project>[] = [
    {
      key: 'nombre',
      label: 'Nombre',
      width: '25%',
    },
    {
      key: 'cliente',
      label: 'Cliente',
      width: '20%',
    },
    {
      key: 'ubicacion',
      label: 'Ubicación',
      width: '15%',
    },
    {
      key: 'duracion',
      label: 'Duración',
      width: '12%',
    },
    {
      key: 'personasInvolucradas',
      label: 'Personas',
      width: '10%',
    },
    {
      key: 'createdAt',
      label: 'Fecha',
      width: '10%',
      render: (value: unknown) => new Date(value as string).toLocaleDateString('es-ES'),
    },
    {
      key: '_id',
      label: 'Acciones',
      width: '8%',
      render: (_value: unknown, row: Project) => (
        <button
          className="btn-action-edit"
          onClick={(e) => {
            e.stopPropagation();
            openEditModal(row);
          }}
          title="Editar"
        >
          <svg 
            width="18" 
            height="18" 
            viewBox="0 0 24 24" 
            fill="none" 
            stroke="currentColor" 
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
          >
            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>
            <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
          </svg>
        </button>
      ),
    },
  ];

  return (
    <div className="projects-section">
      <div className="section-header">
        <h2>Gestión de Proyectos</h2>
        <div className="header-actions">
          <button className="btn-refresh" onClick={loadProjects} disabled={loading}>
            <svg 
              width="16" 
              height="16" 
              viewBox="0 0 24 24" 
              fill="none" 
              stroke="currentColor" 
              strokeWidth="2"
              style={{ marginRight: '8px' }}
            >
              <polyline points="23 4 23 10 17 10"></polyline>
              <polyline points="1 20 1 14 7 14"></polyline>
              <path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"></path>
            </svg>
            Actualizar
          </button>
          <button className="btn-create" onClick={openCreateModal}>
            <svg 
              width="16" 
              height="16" 
              viewBox="0 0 24 24" 
              fill="none" 
              stroke="currentColor" 
              strokeWidth="2"
              style={{ marginRight: '8px' }}
            >
              <line x1="12" y1="5" x2="12" y2="19"></line>
              <line x1="5" y1="12" x2="19" y2="12"></line>
            </svg>
            Crear Proyecto
          </button>
        </div>
      </div>

      {error && <div className="error-message">{error}</div>}

      <div className="content-layout">
        <Table
          columns={columns}
          data={projects}
          loading={loading}
          emptyMessage="No hay proyectos registrados"
          onRowClick={setSelectedProject}
        />
      </div>

      {/* Modal de Detalles */}
      <Modal
        isOpen={!!selectedProject}
        onClose={() => setSelectedProject(null)}
        title="Detalles del Proyecto"
        size="large"
      >
        {selectedProject && (
          <div className="project-detail-modal">
            <div className="detail-grid">
              <div className="detail-field">
                <label>Nombre:</label>
                <span>{selectedProject.nombre}</span>
              </div>

              <div className="detail-field">
                <label>Cliente:</label>
                <span>{selectedProject.cliente}</span>
              </div>

              <div className="detail-field">
                <label>Ubicación:</label>
                <span>{selectedProject.ubicacion}</span>
              </div>

              <div className="detail-field">
                <label>Duración:</label>
                <span>{selectedProject.duracion}</span>
              </div>

              <div className="detail-field">
                <label>Personas Involucradas:</label>
                <span>{selectedProject.personasInvolucradas}</span>
              </div>

              <div className="detail-field">
                <label>Likes:</label>
                <span>{selectedProject.likes}</span>
              </div>

              <div className="detail-field full-width">
                <label>Descripción:</label>
                <p className="description-content">{selectedProject.descripcion}</p>
              </div>

              {selectedProject.imagenes.length > 0 && (
                <div className="detail-field full-width">
                  <label>Imágenes ({selectedProject.imagenes.length}):</label>
                  <div className="images-grid">
                    {selectedProject.imagenes.map((img) => (
                      <img
                        key={img._id}
                        src={img.url}
                        alt={img.filename}
                        className="project-image-thumbnail"
                      />
                    ))}
                  </div>
                </div>
              )}

              <div className="detail-field">
                <label>Creado por:</label>
                <span>{selectedProject.createdBy.username}</span>
              </div>

              <div className="detail-field">
                <label>Fecha de creación:</label>
                <span>{new Date(selectedProject.createdAt).toLocaleString('es-ES')}</span>
              </div>
            </div>

            <div className="detail-actions">
              <button
                className="btn-action btn-edit"
                onClick={() => {
                  setSelectedProject(null);
                  openEditModal(selectedProject);
                }}
              >
                Editar
              </button>
              <button
                className="btn-action btn-delete"
                onClick={() => {
                  handleDelete(selectedProject._id);
                  setSelectedProject(null);
                }}
              >
                Eliminar
              </button>
            </div>
          </div>
        )}
      </Modal>

      {/* Modal de Formulario (Crear/Editar) */}
      <Modal
        isOpen={showFormModal}
        onClose={() => setShowFormModal(false)}
        title={editingProject ? 'Editar Proyecto' : 'Crear Proyecto'}
        size="large"
      >
        <form onSubmit={handleSubmit} className="project-form">
          <div className="form-grid">
            <div className="form-field">
              <label htmlFor="nombre">
                Nombre del Proyecto <span className="required">*</span>
              </label>
              <input
                id="nombre"
                type="text"
                value={formData.nombre}
                onChange={(e) => setFormData({ ...formData, nombre: e.target.value })}
                className={formErrors.nombre ? 'error' : ''}
                maxLength={200}
              />
              {formErrors.nombre && <span className="error-message">{formErrors.nombre}</span>}
            </div>

            <div className="form-field">
              <label htmlFor="cliente">
                Cliente <span className="required">*</span>
              </label>
              <input
                id="cliente"
                type="text"
                value={formData.cliente}
                onChange={(e) => setFormData({ ...formData, cliente: e.target.value })}
                className={formErrors.cliente ? 'error' : ''}
                maxLength={200}
              />
              {formErrors.cliente && <span className="error-message">{formErrors.cliente}</span>}
            </div>

            <div className="form-field">
              <label htmlFor="ubicacion">
                Ubicación <span className="required">*</span>
              </label>
              <input
                id="ubicacion"
                type="text"
                value={formData.ubicacion}
                onChange={(e) => setFormData({ ...formData, ubicacion: e.target.value })}
                className={formErrors.ubicacion ? 'error' : ''}
                maxLength={200}
              />
              {formErrors.ubicacion && <span className="error-message">{formErrors.ubicacion}</span>}
            </div>

            <div className="form-field">
              <label htmlFor="duracion">
                Duración <span className="required">*</span>
              </label>
              <input
                id="duracion"
                type="text"
                value={formData.duracion}
                onChange={(e) => setFormData({ ...formData, duracion: e.target.value })}
                className={formErrors.duracion ? 'error' : ''}
                maxLength={100}
                placeholder="Ej: 6 meses, 1 año"
              />
              {formErrors.duracion && <span className="error-message">{formErrors.duracion}</span>}
            </div>

            <div className="form-field">
              <label htmlFor="personasInvolucradas">
                Personas Involucradas <span className="required">*</span>
              </label>
              <input
                id="personasInvolucradas"
                type="number"
                min="1"
                value={formData.personasInvolucradas}
                onChange={(e) => setFormData({ ...formData, personasInvolucradas: parseInt(e.target.value) || 1 })}
                className={formErrors.personasInvolucradas ? 'error' : ''}
              />
              {formErrors.personasInvolucradas && <span className="error-message">{formErrors.personasInvolucradas}</span>}
            </div>

            <div className="form-field full-width">
              <label htmlFor="descripcion">
                Descripción <span className="required">*</span>
              </label>
              <textarea
                id="descripcion"
                value={formData.descripcion}
                onChange={(e) => setFormData({ ...formData, descripcion: e.target.value })}
                className={formErrors.descripcion ? 'error' : ''}
                maxLength={2000}
                rows={5}
              />
              {formErrors.descripcion && <span className="error-message">{formErrors.descripcion}</span>}
              <span className="char-count">{formData.descripcion.length}/2000</span>
            </div>
          </div>

          <div className="form-actions">
            <button
              type="button"
              className="btn-cancel"
              onClick={() => setShowFormModal(false)}
              disabled={submitting}
            >
              Cancelar
            </button>
            <button
              type="submit"
              className="btn-submit"
              disabled={submitting}
            >
              {submitting ? 'Guardando...' : editingProject ? 'Actualizar' : 'Crear'}
            </button>
          </div>
        </form>
      </Modal>
    </div>
  );
}
